...
- низкая точность при zero-shor learning (<0.1).
...
- padding
- max_length
- return_tensors="pt или tf" ('Pytorch(cuda() или cpu()) или TensorFLow(cuda()), разные форматы представления векторизированных данных в тензорном виде, первый используеться для обучения и тестирования моделей, второй служуит больше форматом отптимизации для выхода чего-то крайне тяжелого в продакшен')
```
```
допишу
```
